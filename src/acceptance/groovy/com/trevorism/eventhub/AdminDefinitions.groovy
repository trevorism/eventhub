...
Given(/the topic {topic} does not exist/) { String topic ->
...
Given(/the topic {topic} already exists or is created/) { String topic ->
    topicUnderTest = topic
    ResponseUtils.closeSilently jsonHttpClient.post("${baseUrl}/admin/topic", topic, ["Authorization":passwordProvider.password])
}

Given(/the subscription {subscription}  already exists or is created/) { String subscription ->
...
When(/the topic {topic} is created$/) { String topic ->
...
When(/the subscription {subscription} is deleted/) { String subscription ->
...
Then(/the topic {topic} cannot be found/) { String topic ->
    Thread.sleep(2000)
...
Then(/the topic {topic} exists/) { String topic ->
    Thread.sleep(2000)
    assert ResponseUtils.getEntity(jsonHttpClient.get("${baseUrl}/admin/topic/", ["Authorization":passwordProvider.password])).contains(topic)
}

Then(/the subscription {subscription} exists/) { String subscription ->
    Thread.sleep(2000)
...
Then(/the subscription {subscription} does not exist/) { String subscription ->
    Thread.sleep(2000)
